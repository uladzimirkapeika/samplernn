...
class Generator(Runner):
    ...
    def __call__(self, n_seqs, seq_len):
        ...
        for i in range(self.model.lookback, self.model.lookback + seq_len):
            for (tier_index, rnn) in reversed(list(enumerate(self.model.frame_level_rnns))):
                ...
                if tier_index == len(self.model.frame_level_rnns) - 1:
                    ...
                    frame_index = (i // rnn.n_frame_samples) % self.model.frame_level_rnns[tier_index + 1].frame_size
                    upper_tier_conditioning = frame_level_outputs[tier_index + 1][:, frame_index, :].unsqueeze(1)
            ...
            if self.cuda:
                prev_samples = prev_samples.cuda()
            upper_tier_conditioning = frame_level_outputs[0][:, i % bottom_frame_size, :].unsqueeze(1)
